...
class TemporalTTAAdapter(BaseAdapter):
    ...
    def adapt_and_detect(self, image: Image.Image, target_classes: List[str],
        ...
        # Initialize SSM
        if self.ssm is None:
            self._initialize_ssm(detection_data['raw_text_embeddings'])
        
        # =====================================================================
        # DIAGNOSTIC: Detailed fusion analysis
        # =====================================================================
        if self.ssm is not None and self.ssm.state.initialized:
            raw_features = detection_data['raw_features']
            ...
            # Get SSM predictions
            proto_probs = self.ssm.predict(raw_features)
        ...
        if self.update_after_nms and self.ssm is not None:
            if len(final_result.get('raw_features', [])) > 0:
                self._update_ssm(
                    final_result['raw_features'],
                    final_result['scores'],
    ...
    def _compare_scores(self, vlm_scores, fused_scores):
        ...
        print(f"   VLM scores > 0.8: {(vlm_scores > 0.8).sum()}")
        print(f"   Fused scores > 0.8: {(fused_scores > 0.8).sum()}")
        ...
        print(f"   Detections boosted above 0.8: {((vlm_scores <= 0.8) & (fused_scores > 0.8)).sum()}")
    
    def _initialize_ssm(self, raw_text_embeddings: np.ndarray):
        """Initialize SSM."""
        self.raw_feature_dim = raw_text_embeddings.shape[1]
        
        print(f"SSM initialized: {self.num_classes} classes, {self.raw_feature_dim}D")
        
        self.ssm = TemporalSSM(
            num_classes=self.num_classes,
            feature_dim=self.raw_feature_dim,
        ...
        self.ssm.initialize_from_text_embeddings(raw_text_embeddings)
        self._prev_prototypes = raw_text_embeddings.copy()
